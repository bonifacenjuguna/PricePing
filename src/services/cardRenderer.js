...
function buildBackgroundSvg({ coin, price, changeUsd, changePct, direction }) {
  ...
  if (!coin.isStable && direction && changePct !== null && changePct !== undefined) {
    ...
    badge = `
      ...
      <text x="${badgeX + badgeWidth / 2}" y="106" font-family="DejaVu Sans, sans-serif"
  ...
  const watermark = `
    <text x="${CARD_WIDTH - 40}" y="${CARD_HEIGHT - 36}" font-family="DejaVu Sans, sans-serif"
...
<svg width="${CARD_WIDTH}" height="${CARD_HEIGHT}" viewBox="0 0 ${CARD_WIDTH} ${CARD_HEIGHT}"
  ...
  <text x="${LOGO_CIRCLE_CX + LOGO_CIRCLE_R + 40}" y="160" font-family="DejaVu Sans, sans-serif"
        font-size="46" font-weight="700" fill="${textColor}">${nameStr}</text>
  <text x="${LOGO_CIRCLE_CX + LOGO_CIRCLE_R + 40}" y="200" font-family="DejaVu Sans, sans-serif"
        font-size="30" font-weight="400" fill="${subTextColor}">${symbolStr}</text>

  <text x="80" y="380" font-family="DejaVu Sans, sans-serif" font-size="96" font-weight="700"
